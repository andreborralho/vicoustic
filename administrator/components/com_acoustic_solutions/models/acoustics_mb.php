...
class Acoustic_solutionsModelacoustics_mb extends JModelList
{
    ...
    public function __construct($config = array())
    {
        if (empty($config['filter_fields'])) {
            $config['filter_fields'] = array(
                ...
                'panel1_boxes', 'a.panel1_boxes',
                'panel2_boxes', 'a.panel2_boxes',
                'panel3_boxes', 'a.panel3boxes',
                'panel4_boxes', 'a.panel4_boxes',
	...
	protected function getListQuery()
	{
		...
		$query->select('acoustic_lines.name AS line_name');
		$query->join('LEFT', '#__acoustic_solution_lines AS acoustic_lines ON line_id = acoustic_lines.id');
