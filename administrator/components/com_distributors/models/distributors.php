...
class DistributorsModeldistributors extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Load the parameters.
		$params = JComponentHelper::getParams('com_distributors');
    ...
    if (is_numeric($published)) {
		...
		if (!empty($search)) {
			if (stripos($search, 'id:') === 0) {
                ...
                $query->where('( a.name LIKE '.$search.'  OR  a.city LIKE '.$search.'  OR  a.country LIKE '.$search.'  OR  a.email LIKE '.$search.'  OR  a.website LIKE '.$search.' )');
			}
		}
