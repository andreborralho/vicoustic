 ...
 * Portfolio model.
 */
class PortfoliosModelportfolio extends JModelAdmin
{
	/**
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_portfolios.edit.portfolio.data', array());
		...
		if (empty($data)) {
			...
			//Support for 'multiple' field
			$data->panels = json_decode($data->panels);
	...
	protected function prepareTable(&$table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__portfolio');
				$max = $db->loadResult();
