...
class plgSystemModulesAnywhereHelper
{
	...
	function processModule($module, $chrome = 'none', $ignores = array(), $overrides = array())
	{
		...
		$db = JFactory::getDBO();
		$where = ' AND ( title=' . $db->quote(NNText::html_entity_decoder($module)) . '';
		if (is_numeric($module)) {
			$where .= ' OR id=' . $module;
		}
		$where .= ' ) ';
		if (!$ignore_access) {
			$where .= ' AND access ' . (defined('_JACL') ? 'IN (' . $this->params->aid_jaclplus . ')' : '<= ' . (int) $this->params->aid);
		}
		if (!$ignore_state) {
		...
		if ($module && !$ignore_assignments && function_exists('plgSystemAdvancedModulesPrepareModuleList')) {
			$module->published = 1;
