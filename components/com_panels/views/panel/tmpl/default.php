...
<?php if( $this->item ) {
	...
	<section id="panels_fields_list" class="products_fields_list">
		...
		<div class="product_portfolio_images">
			<?php
				...
				foreach ($this->items as $i => $item) {
					var_dump($item->portfolio_photo_id1);
					...
					echo PanelsHelper::renderProductPortfolioImage($this->item->portfolio_photo_id3, $item->portfolio_photo_id3, $item->portfolio_photo3_thumbnail, $item->portfolio_photo3_label);
				}
			?>
		</div>
			...
			<table class="product_similar_table">
				...
				<tr>
					...
					<?php if($this->item->wood == 1) { ?>
						<th><?php echo JText::_('WOOD_COLOR'); ?></th>
					<?php } ?>

					<?php if($this->item->metal == 1) { ?>
						<th><?php echo JText::_('METAL_COLOR'); ?></th>
					<?php } ?>

					<?php if($this->item->fabric == 1) { ?>
						<th><?php echo JText::_('FABRIC_COLOR'); ?></th>
					<?php } ?>

					<?php if($this->item->polystyrene == 1) { ?>
						<th><?php echo JText::_('POLYSTYRENE_COLOR'); ?></th>
					<?php } ?>

					<th><?php echo JText::_('ABSORPTION_FREQUENCY'); ?></th>
					<th><?php echo JText::_('AW'); ?></th>
					<th><?php echo JText::_('NRC'); ?></th>
				</tr>
				</thead>
				...
				<?php foreach ($this->items as $i => $item) {
					if ($item->family == $this->item->family) { ?>
						<tr>
							<td><?php echo PanelsHelper::renderSimilarProductImage($item, JRoute::_('index.php?option=com_panels&view=panel&id=' . (int) $item->id)); ?></td>
							<td><?php echo $item->ref; ?></td>

							<?php if($this->item->wood == 1) { ?>
								<td><?php echo ucwords($item->wood_color); ?></td>
							<?php } ?>

							<?php if($this->item->metal == 1) { ?>
								<td><?php echo ucwords($item->metal_color); ?></td>
							<?php } ?>

							<?php if($this->item->fabric == 1) { ?>
								<td><?php echo ucwords($item->fabric_color); ?></td>
							<?php } ?>

							<?php if($this->item->polystyrene == 1) { ?>
								<td><?php echo ucwords($item->polystyrene_color); ?></td>
							<?php } ?>

							<td><?php echo ucwords($item->absorption_frequency); ?></td>
							<td><?php echo PanelsHelper::renderIconsProperty($item->aw); ?></td>
							<td><?php echo PanelsHelper::renderIconsProperty($item->nrc); ?></td>
						</tr>
					<?php
					}
				}
	...
	if($this->item->edge_leveled == 1 || $this->item->edge_angled == 1) {
		...
		if($this->item->edge_leveled == 1) {
			$pdf->Cell(11.5, 5, JText::_('EDGE_LEVELED'));
		...
		if($this->item->edge_angled == 1) {
			$pdf->Cell(13, 5, JText::_('EDGE_ANGLED'));
			$pdf->Ln();
		}
	}
	...
	if($this->item->installation_wall == 1 || $this->item->installation_ceiling == 1 || $this->item->installation_floor == 1 || $this->item->installation_corner == 1) {
		$pdf->Cell(10, 5, JText::_('INSTALLATION_PLACE') . ": ", 'L');

		if($this->item->installation_wall == 1) {
			$pdf->Cell(6, 5, JText::_('INSTALLATION_WALL'));
		...
		if($this->item->installation_ceiling == 1) {
			$pdf->Cell(10, 5, JText::_('INSTALLATION_CEILING'));
		...
		if($this->item->installation_floor == 1) {
			$pdf->Cell(7, 5, JText::_('INSTALLATION_FLOOR'));
		...
		if($this->item->installation_corner == 1) {
			$pdf->Cell(8, 5, JText::_('INSTALLATION_CORNER'));
		}
