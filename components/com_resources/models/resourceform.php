 ...
 * Resources model.
 */
class ResourcesModelResourceForm extends JModelForm
{
	...
	{
		$app = JFactory::getApplication('com_resources');

		// Load state from the request userState on edit or from the passed variable on default
        if (JFactory::getApplication()->input->get('layout') == 'edit') {
            $id = JFactory::getApplication()->getUserState('com_resources.edit.resource.id');
        } else {
            $id = JFactory::getApplication()->input->get('id');
            JFactory::getApplication()->setUserState('com_resources.edit.resource.id', $id);
        }
	...
	public function getTable($type = 'Resource', $prefix = 'ResourcesTable', $config = array())
	{   
	...
	{
		// Get the form.
		$form = $this->loadForm('com_resources.resource', 'resourceform', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	public function save($data)
	{
        ...
        if($id) {
            ...
            $authorised = $user->authorise('core.create', 'com_resources');
            if($user->authorise('core.edit.state', XXX_ASSET_NAME_XXX) !== true && $state == 1){ //The user cannot edit the state of the item.
                $data['state'] = 0;
            }
        }
