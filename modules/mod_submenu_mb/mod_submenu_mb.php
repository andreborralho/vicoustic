...
$acoustic_treatment_id = 9801;
$insulation_id = 9805;
$accessories_id = 9809;
$products_title_id = 11735;
$tools_id = 9769;

$slideshow_parent_id = 10603;
...
foreach($menu as $item){
	...
	($item->id == $products_title_id && $parentId == $products_parent_id) && $parentId != $slideshow_parent_id){
		...
		//separator
		if($url == "" || $url == "/"){
			echo '<li class="item-'.$item->id.'"><div class="separator">'.$item->title.'</div></li>';
	...
	($item->parent_id != $insulation_id && $parentId != $products_parent_id) && $parentId != $slideshow_parent_id){
	...
	//echo $item->id . " ss". $parentId. " aa".$item->parent_id;
	//SLIDESHOW MENU
    elseif($item->level == 3 && $parentId == $slideshow_parent_id){		
