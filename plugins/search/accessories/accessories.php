...
/**
 * Accessories Search plugin
 *
 * @package		Joomla.Plugin
 * @subpackage	Search.contacts
 * @since		1.6
 */
class plgSearchAccessories extends JPlugin
{	
	/**	 
	 * Constructor
	 *
	...
	*/
	...
	{
		static $areas = array(
			'accessories' => 'PLG_SEARCH_ACCESSORIES_ACCESSORIES'
		);
		return $areas;
	}

	/**
	* Accessories Search method
	*
		...
		if (!empty($state)) {
			...
			if ($rows) {
				foreach($rows as $key => $row) {
					$rows[$key]->href = $tokens[3] . '/products/accessories/accessory/'.$row->id;
					$rows[$key]->component = JText::_('PLG_SEARCH_ACCESSORIES_ACCESSORIES');
