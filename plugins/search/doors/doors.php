 ...
 * @since		1.6
 */
class plgSearchDoors extends JPlugin
{	
	/**	 
	 * Constructor
	 *
		...
		if (!empty($state)) {
			...
			if ($rows) {
				foreach($rows as $key => $row) {
					$rows[$key]->href = $tokens[3] . '/products/insulation/doors/door/'.$row->id;
					$rows[$key]->component = JText::_('PLG_SEARCH_DOORS_DOORS');
