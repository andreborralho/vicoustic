...
/**
 * Blankets Search plugin
 *
 * @package		Joomla.Plugin
 * @subpackage	Search.contacts
 * @since		1.6
 */
class plgSearchBlankets extends JPlugin
{	
	/**	 
	 * Constructor
	 *
	...
	*/
	...
	{
		static $areas = array(
			'blankets' => 'PLG_SEARCH_BLANKETS_BLANKETS'
		);
		return $areas;
	}

	/**
	* Blankets Search method
	*
		...
		if (!empty($state)) {
			...
			if ($rows) {
				foreach($rows as $key => $row) {
					$rows[$key]->href = $tokens[3] . '/products/insulation/blankets/blanket/'.$row->id;
					$rows[$key]->component = JText::_('PLG_SEARCH_BLANKETS_BLANKETS');
